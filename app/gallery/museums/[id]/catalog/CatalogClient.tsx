...
export default function CatalogClient({ museumId }: { museumId: number }) {
  ...
  const printable = Boolean(museum?.contentAvailable && museum.artworks.length && loadedImages.size === museum.artworks.length && !imageFailed);
  useEffect(() => {
  ...
  return (
    <main id="main-content" tabIndex={-1} className="min-h-screen bg-[#f5f0e6] px-6 py-8 text-[#171816] print:px-0 print:py-0">
      <div className="mx-auto max-w-5xl">
        <nav className="mb-8 flex flex-wrap items-center justify-between gap-3 print:hidden"><Link href={`/gallery/museums/${museumId}`}>전시로 돌아가기</Link><button type="button" disabled={!printable} onClick={() => window.print()} className="min-h-11 bg-[#171816] px-5 py-2 text-sm text-white disabled:opacity-40">도록 인쇄 / PDF</button></nav>
        {museum && museum.contentAvailable && museum.artworks.length > 0 ? (
          <>
            <header className="grid min-h-[60vh] content-between border-y border-[#171816] py-10">
              ...
              <div><h1 className="break-words font-[var(--font-display)] text-4xl sm:text-6xl md:text-8xl">{museum.name}</h1><p className="mt-4 text-lg">{museum.ownerName}</p><p className="mt-8 max-w-2xl leading-8 text-black/65">{museum.curatorNote || museum.description}</p></div>
            ...
            <section className="mt-16 space-y-20">
              {museum.artworks.map((artwork, index) => (
                <article key={artwork.museumArtworkId} className="break-inside-avoid border-t border-black/30 pt-6 md:grid md:grid-cols-[1.35fr_.65fr] md:gap-10">
                  <div className="relative aspect-[4/3]"><Image src={artwork.imageUrl} alt={artwork.title} fill sizes="(min-width: 768px) 60vw, 100vw" loading="eager" onLoad={() => setLoadedImages((previous) => new Set(previous).add(artwork.museumArtworkId))} onError={() => setImageFailed(true)} className="object-contain object-left" /></div>
        ...
        ) : query.isLoading ? <QueryState kind="loading" title="도록을 준비하고 있습니다" /> : query.data?.error ? <QueryState kind="error" title="도록을 불러오지 못했습니다" description={query.data.error} retry={() => void query.refetch()} /> : <QueryState title="공개된 전시 작품이 없습니다" description="전시가 공개되면 도록을 확인할 수 있습니다." />}
        {imageFailed ? <p role="alert" className="mt-5 print:hidden">일부 작품 이미지를 불러오지 못했습니다. 새로고침 후 이미지가 모두 표시되면 인쇄할 수 있습니다.</p> : null}
      </div>
    </main>
  );
}
